...
func init() {
	android.RegisterModuleType("dotFE_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = dotFEExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = dotFEExpandVariables(ctx, dep_file)
	...
	cmd := dotFEExpandVariables(ctx, String(g.properties.Cmd))
